...
export const Route = createFileRoute("/devices")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Monitor device sync status, resolve conflicts and manage assigned scanning devices." },
      { property: "og:title", content: "Devices — synchronisation centre" },
      { property: "og:description", content: "Device sync status, pending records, conflicts and history." },
    ],
  }),
...
function DevicesPage() {
  ...
  const historyQuery = useQuery({ queryKey: ["device-history"], queryFn: () => deviceService.history(), enabled: !!historyFor });

  const rows = useMemo(() => {
    let list = devicesQuery.data ?? [];
    const q = search.trim().toLowerCase();
    if (q) list = list.filter((d) => `${d.name} ${d.assignedUser} ${d.location}`.toLowerCase().includes(q));
  ...
  const runSync = async (device: Device) => {
    ...
    try {
      ...
      toast.success("Synchronisation complete", { description: `${device.name} is now up to date.` });
    } finally {
      setSyncingId(null);
    }
  };
  ...
  return (
    <AppShell permission="devices.view" area="device management">
      <PageHeader
        title="Devices"
        description={devicesQuery.isLoading ? "Loading devices…" : `${rows.length} devices match the current filters.`}
        actions={
          <Button
            size="sm"
            className="h-8 text-[12px]"
            onClick={() => toast("Synchronising all devices", { description: "This may take a few minutes for devices with large pending queues." })}
          >
      ...
      <div className="surface-card overflow-hidden">
        <div className="flex flex-wrap items-center gap-2 border-b border-border px-3 py-2.5">
          <div className="relative min-w-[220px] flex-1">
            ...
            <Input aria-label="Search devices" placeholder="Search by device, user or location" className="h-8 pl-8 text-[13px]" value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
          <Select value={status} onValueChange={setStatus}>
            <SelectTrigger className="h-8 w-auto min-w-[130px] text-[12px]" aria-label="Status"><SelectValue placeholder="Status" /></SelectTrigger>
            <SelectContent>
        ...
        {devicesQuery.isError ? (
          ...
          <EmptyState title="No devices match these filters" description="Adjust the search or filters to see more devices." />
        ) : (
            ...
            <table className="w-full min-w-[1180px] border-collapse text-left text-[12.5px]">
              <thead className="sticky top-14 z-10 bg-card">
                <tr className="border-b border-border">
                  <Th>Device</Th><Th>Type</Th><Th>Assigned user</Th><Th>Location</Th><Th>Status</Th>
                  <Th>Last sync</Th><Th>Pending</Th><Th>Failed</Th><Th>Conflicts</Th><Th>Version</Th><Th>Last activity</Th>
                  ...
                  <tr key={d.id} className="border-b border-border/70 hover:bg-accent/50">
                    ...
                    <td className="px-3 py-2 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end gap-1">
                        <Button variant="outline" size="sm" className="h-7 text-[11.5px]" disabled={syncingId === d.id} onClick={() => void runSync(d)}>
                          <RefreshCw className={cn("h-3 w-3", syncingId === d.id && "animate-spin")} /> {d.status === "failed" ? "Retry" : "Sync now"}
                        </Button>
                        {d.conflicts > 0 ? (
                          <Button variant="outline" size="sm" className="h-7 text-[11.5px]" onClick={() => setConflictsFor(d)}>
                            <AlertTriangle className="h-3 w-3" /> Conflicts
                          </Button>
                        ) : null}
                        <Button variant="ghost" size="icon" className="h-7 w-7" aria-label={`History for ${d.name}`} onClick={() => setHistoryFor(d)}>
                          <History className="h-3.5 w-3.5" />
                        </Button>
                        {d.status !== "disabled" ? (
                          <Button variant="ghost" size="icon" className="h-7 w-7 text-danger" aria-label={`Disable ${d.name}`} onClick={() => setDisableFor(d)}>
      ...
      <Sheet open={!!conflictsFor} onOpenChange={(o) => !o && setConflictsFor(null)}>
        <SheetContent side="right" className="w-full overflow-y-auto sm:max-w-md">
          <SheetHeader><SheetTitle>Conflicts — {conflictsFor?.name}</SheetTitle></SheetHeader>
          <div className="space-y-2 px-4 pb-4">
            {Array.from({ length: conflictsFor?.conflicts ?? 0 }).map((_, i) => (
              <div key={i} className="rounded-lg border border-border p-3">
                <p className="text-[12.5px] font-medium">Attendance record #{1000 + i} — conflicting scan time</p>
                <p className="mt-1 text-[11.5px] text-muted-foreground">Local record differs from the server copy for this occasion.</p>
                <div className="mt-2 flex justify-end">
                  <Button
                    size="sm"
                    className="h-7 text-[11.5px]"
                    onClick={() => toast.success("Conflict resolved", { description: "The server copy has been kept as the source of truth." })}
                  >
                    Resolve conflict
                  </Button>
                </div>
      ...
      <Dialog open={!!historyFor} onOpenChange={(o) => !o && setHistoryFor(null)}>
        <DialogContent className="max-w-lg">
          ...
          <div className="max-h-[360px] space-y-2 overflow-y-auto pr-1">
            {(historyQuery.data ?? []).filter((h) => h.deviceId === historyFor?.id).map((h) => (
              <div key={h.id} className="flex items-center justify-between rounded-md bg-muted px-2.5 py-1.5 text-[12px]">
                <span>{h.at} · {h.records} records</span>
                <StatusBadge status={h.result === "success" ? "synced" : h.result === "failed" ? "failed" : "pending"} />
              </div>
            ))}
          </div>
        </DialogContent>
      ...
      <AlertDialog open={!!disableFor} onOpenChange={(o) => !o && setDisableFor(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This device will no longer be able to submit attendance scans until re-enabled.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction
              className="bg-danger text-danger-foreground hover:bg-danger/90"
              onClick={() => { toast.success("Device disabled", { description: `${disableFor?.name} has been disabled.` }); setDisableFor(null); }}
            >
              Disable device
            </AlertDialogAction>
          </AlertDialogFooter>
