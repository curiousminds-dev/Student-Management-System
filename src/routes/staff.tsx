  ...
  head: () => ({
    meta: [
      { title: "Staff — accounts, roles and device assignment" },
      {
        name: "description",
        content:
          "Manage staff accounts, roles, permissions, assigned classes and devices at Nile Crest Secondary School.",
      },
      { property: "og:title", content: "Staff — accounts, roles and device assignment" },
      {
        property: "og:description",
        content: "Staff accounts, roles, permissions and device assignment.",
      },
    ],
  }),
  component: StaffPage,
...
function StaffPage() {
  ...
  const devicesQuery = useQuery({
    queryKey: ["devices-for-staff"],
    queryFn: () => deviceService.list(),
  });

  const roles = (rolesQuery.data ?? []) as Role[];
  const departments = useMemo(
    () => Array.from(new Set((staffQuery.data ?? []).map((s) => s.department))),
    [staffQuery.data],
  );
  ...
  const rows = useMemo(() => {
    ...
    if (q)
      list = list.filter((s) => `${s.name} ${s.staffNumber} ${s.email}`.toLowerCase().includes(q));
  ...
  return (
    <AppShell permission="staff.manage" area="staff management">
      <PageHeader
        ...
        description={
          staffQuery.isLoading
            ...
            : `${rows.length} staff members match the current filters.`
        }
        actions={
          manage ? (
        ...
        <div className="surface-card min-w-0 overflow-hidden">
          ...
          {staffQuery.isError ? (
            ...
            <EmptyState
              ...
              description="Adjust the search or filters to see more staff members."
            />
          ) : (
                      ...
                      >
                        <td className="px-3 py-2">
                          <div className="flex min-w-0 items-center gap-2.5">
                            <LearnerAvatar
                              name={s.name}
                              hue={(s.staffNumber.charCodeAt(0) * 7) % 360}
                              size={30}
                              ring={isSelected}
                            />
                        ...
                        <td className="px-3 py-2">
                          {s.assignedClasses.length ? s.assignedClasses.join(", ") : "—"}
                        </td>
                        <td className="px-3 py-2 whitespace-nowrap">
                          {s.assignedDeviceName ?? "None"}
                        </td>
                        <td className="px-3 py-2">
                          {isSelected ? (
                                    ...
                                    <DropdownMenuItem
                                      onClick={() =>
                                        toast.success("Invitation resent", {
                                          description: `A new invitation email was sent to ${s.email}.`,
                                        })
                                      }
                                    >
                                    ...
                                    <DropdownMenuItem
                                      onClick={() =>
                                        toast.success("Password reset sent", {
                                          description: `A reset link was sent to ${s.email}.`,
                                        })
                                      }
                                    >
                                    ...
                                    <DropdownMenuItem
                                      onClick={() =>
                                        toast.success("Account reactivated", {
                                          description: `${s.name}'s account can now sign in.`,
                                        })
                                      }
                                    >
          ...
          {panelOpen ? (
            <div className="sticky top-[72px]">
              <StaffPanel
                ...
                onPermissions={() => selected && setPermsFor(selected)}
                onActivity={() => selected && setActivityFor(selected)}
              />
            </div>
          ) : (
            <Button
      ...
      <SectionCard
        ...
        description="Permissions granted to each staff role"
        className="mt-4"
      >
          ...
          <div className="max-h-[420px] overflow-auto">
            <table className="w-full min-w-[900px] border-collapse text-left text-[12px]">
              <thead className="sticky top-0 z-10 bg-card">
                <tr className="border-b border-border">
                  <th className="sticky left-0 z-20 bg-card px-3 py-2 text-[11px] font-semibold tracking-wide text-muted-foreground uppercase">
                    Permission
                  </th>
                  {roles.map((r) => (
                    <th
                      key={r.key}
                      className="px-3 py-2 text-center text-[11px] font-semibold tracking-wide text-muted-foreground uppercase whitespace-nowrap"
                    >
                      {r.name}
                    </th>
                  ))}
              ...
              <tbody>
                {PERMISSION_ORDER.map((p) => (
                  <tr key={p} className="border-b border-border/70">
                    <td className="sticky left-0 bg-card px-3 py-1.5 font-medium whitespace-nowrap">
                      {PERMISSION_LABELS[p]}
                    </td>
                    {roles.map((r) => (
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Create a new staff account and send an invitation email.
            </DialogDescription>
          </DialogHeader>
          <form
            className="space-y-3"
            onSubmit={(e) => {
              ...
              toast.success("Staff member added", {
                description: "An invitation email has been sent to set up their account.",
              });
            }}
          >
            <div className="space-y-1.5">
              ...
              <Input
                id="staff-name"
                required
                placeholder="e.g. Patricia Nabukalu"
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                type="email"
                required
                placeholder="p.nabukalu@nilecrest.ac.ug"
                className="h-8 text-[13px]"
              />
            </div>
            <div className="space-y-1.5">
      ...
      <Dialog open={!!roleDialogFor} onOpenChange={(o) => !o && setRoleDialogFor(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Update the role assigned to {roleDialogFor?.name}. This changes their permissions
              immediately.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setRoleDialogFor(null)}>
              Cancel
            </Button>
            <Button
              onClick={() => {
                toast.success("Role updated", {
                  description: `${roleDialogFor?.name}'s role has been changed.`,
                });
      ...
      <Dialog open={!!deviceDialogFor} onOpenChange={(o) => !o && setDeviceDialogFor(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Assign a registered device to {deviceDialogFor?.name}.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeviceDialogFor(null)}>
              Cancel
            </Button>
            <Button
              onClick={() => {
                toast.success("Device assigned", {
                  description: `A device has been assigned to ${deviceDialogFor?.name}.`,
                });
      ...
      <Dialog open={!!activityFor} onOpenChange={(o) => !o && setActivityFor(null)}>
        <DialogContent className="max-w-md">
          ...
          <ul className="space-y-2 text-[12.5px]">
            <li className="rounded-md bg-muted px-2.5 py-1.5">
              Signed in from {activityFor?.assignedDeviceName ?? "a school device"} —{" "}
              {activityFor?.lastLogin ?? "no recent sign-in"}
            </li>
            <li className="rounded-md bg-muted px-2.5 py-1.5">
              Recorded attendance for {activityFor?.assignedClasses.join(", ") || "assigned duties"}
            </li>
            <li className="rounded-md bg-muted px-2.5 py-1.5">
              Viewed learner records within {activityFor?.department}
            </li>
          </ul>
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={!!suspendFor} onOpenChange={(o) => !o && setSuspendFor(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              They will immediately lose access to the system until the account is reactivated by an
              administrator.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => {
                toast.success("Account suspended", {
                  description: `${suspendFor?.name} can no longer sign in.`,
                });
                setSuspendFor(null);
              }}
            >
    ...
    return (
      <div className={cn(!embedded && "surface-card")}>
        <EmptyState
          ...
          description="Select a row to see the staff member's profile, role and assigned device."
        />
      </div>
    );
  }
    ...
    <div className={cn("overflow-hidden", !embedded && "surface-card")}>
      <div className="relative border-b border-border px-4 pt-6 pb-4 text-center">
        ...
        <div className="mt-3 flex justify-center">
          <LearnerAvatar
            name={staff.name}
            hue={(staff.staffNumber.charCodeAt(0) * 7) % 360}
            size={96}
          />
        </div>
        <h2 className="mt-3 text-[16px] font-semibold">{staff.name}</h2>
        <p className="text-[12px] text-muted-foreground">
          {staff.roleName} · {staff.department}
        </p>
      ...
      <div className="border-t border-border px-4 py-3">
        ...
        <p className="text-[12.5px]">
          {staff.assignedClasses.length ? staff.assignedClasses.join(", ") : "No classes assigned"}
        </p>
      </div>
    </div>
  );
