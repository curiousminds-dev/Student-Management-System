  ...
  head: () => ({
    meta: [
      { title: "Learner profile — Nile Crest SAPWMS" },
      {
        name: "description",
        content:
          "Full learner profile with attendance, academics, conduct, welfare and QR credential management.",
      },
      { property: "og:title", content: "Learner profile — Nile Crest SAPWMS" },
      {
        property: "og:description",
        content:
          "Attendance, academics, conduct, welfare and QR credential management for a learner.",
      },
    ],
  }),
  notFoundComponent: () => (
    <AppShell>
      <EmptyState
        ...
        description="This learner record does not exist or may have been removed."
        icon={AlertTriangle}
      />
    </AppShell>
  ),
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-border px-6 py-14 text-center">
      ...
      <p className="max-w-sm text-[13px] text-muted-foreground">
        Your role does not have permission to view this information. Ask a school administrator if
        you need access.
      </p>
    </div>
  );
...
const reasonSchema = z.object({
  reason: z.string().min(10, "Provide at least 10 characters explaining the reason"),
});
    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        ...
        <div className="rounded-md bg-muted/50 px-3 py-2 text-[12px] text-muted-foreground">
          Responsible staff: <span className="font-medium text-foreground">{staff}</span> ·{" "}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Textarea rows={3} placeholder="Explain why this action is required" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
...
function LearnerProfilePage() {
  ...
  const revokeMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("QR credential revoked", {
        description: "The learner's card can no longer be used to scan attendance.",
      });
      setRevokeOpen(false);
  ...
  const replaceMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("New QR credential issued", {
        description: "A replacement card has been generated.",
      });
  ...
  if (learnerQuery.isError || !learnerQuery.data) {
    return (
      <AppShell permission={["learners.view", "learners.identity_only"]} area="learner profiles">
        <ErrorState
          message="This learner could not be found or the record failed to load."
          onRetry={() => void learnerQuery.refetch()}
        />
      </AppShell>
    );
  }
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-[12px] text-muted-foreground">
                ...
                <span>
                  {learner.className} · {learner.stream}
                </span>
                <span className="inline-flex items-center gap-1">
                  <Bus className="h-3 w-3" /> {learner.residence}
                  {learner.dormitory ? ` · ${learner.dormitory}` : ""}
                </span>
              </p>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {can("learners.manage") ? (
              <Button
                ...
                onClick={() =>
                  toast("Edit profile", { description: "Opening the learner edit form." })
                }
              >
            ...
            {can("observations.create") ? (
              <Button
                ...
                className="h-8 text-[12px]"
                onClick={() =>
                  toast("Record observation", {
                    description: `New observation form opened for ${learner.fullName}.`,
                  })
                }
              >
            ...
            <Button
              ...
              className="h-8 text-[12px]"
              onClick={() =>
                toast.success("Preparing summary", {
                  description: "The printable learner summary will open shortly.",
                })
              }
            >
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem
                  onClick={() =>
                    toast("Message guardian", {
                      description: `SMS composer opened for ${learner.guardian.name}.`,
                    })
                  }
                >
                  Message guardian
                ...
                <DropdownMenuItem
                  onClick={() =>
                    toast("Transfer requested", {
                      description: "A transfer request draft has been created.",
                    })
                  }
                >
                  Request transfer
                ...
                <DropdownMenuItem
                  ...
                  onClick={() =>
                    toast.error("Deactivate learner", {
                      description: "This requires administrator confirmation.",
                    })
                  }
                >
                  Deactivate learner
        ...
        <Tabs defaultValue="overview">
          <TabsList className="flex h-auto flex-wrap gap-1 bg-muted/60 p-1">
            {[
              ...
              ["conduct", "Conduct"],
              ["welfare", "Welfare"],
              ["health", "Health encounters"],
              ["participation", "Participation"],
              ["interventions", "Interventions"],
              ["documents", "Documents"],
              ["timeline", "Timeline"],
            ].map(([v, l]) => (
              <TabsTrigger key={v} value={String(v)} className="text-[12px]">
                {l}
              </TabsTrigger>
            ))}
          </TabsList>
          ...
          <TabsContent value="overview" className="mt-4">
            <OverviewTab
              learner={learner}
              credentials={credentialsQuery.data as CredentialView[] | undefined}
          ...
          <TabsContent value="academics" className="mt-4">
            {can("academics.view") ? (
              <AcademicsTab className={learner.className} />
            ) : (
              <RestrictedPanel area="Academics" />
            )}
          </TabsContent>
          <TabsContent value="conduct" className="mt-4">
            {can("cases.view") ? (
              <ConductTab learnerId={id} />
            ) : (
              <RestrictedPanel area="Conduct cases" />
            )}
          </TabsContent>
          <TabsContent value="welfare" className="mt-4">
            {can("welfare.view") ? (
              <WelfareTab learnerId={id} />
            ) : (
              <RestrictedPanel area="Welfare records" />
            )}
          </TabsContent>
          <TabsContent value="health" className="mt-4">
            {can("health.view") ? (
              <HealthTab learnerId={id} />
            ) : (
              <RestrictedPanel area="Health encounters" />
            )}
          </TabsContent>
          ...
          <TabsContent value="interventions" className="mt-4">
            {can("welfare.view") ? (
              <InterventionsTab learnerId={id} />
            ) : (
              <RestrictedPanel area="Interventions" />
            )}
          </TabsContent>
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Absence authorised", {
        description: `${learnerName}'s absence has been recorded and guardians notified.`,
      });
      onOpenChange(false);
      form.reset();
    },
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      rows={2}
                      placeholder="e.g. Attending a family introduction ceremony"
                      {...field}
                    />
                  </FormControl>
                  <FormMessage />
...
function QrPlaceholder({ seed, size = 96 }: { seed: string; size?: number }) {
  return (
    <div
      className="grid shrink-0 place-items-center rounded-lg border border-border bg-card"
      style={{ width: size, height: size }}
    >
      <div
        className="grid grid-cols-6 grid-rows-6 gap-0.5 p-1"
      ...
      >
        {Array.from({ length: 36 }).map((_, i) => (
          <span
            ...
            className={cn(
              "rounded-[1px]",
              (i * 7 + seed.length * 3) % 3 === 0 ? "bg-foreground" : "bg-muted",
            )}
          />
        ))}
      </div>
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_340px]">
      <SectionCard
        ...
        description="Primary points of contact for this learner"
      >
        <div className="grid gap-4 p-4 sm:grid-cols-2">
          <div>
            ...
            {learner.guardian.email ? (
              <p className="text-[12px] text-muted-foreground">{learner.guardian.email}</p>
            ) : null}
          </div>
          <div>
            ...
            <p className="text-[12px] text-muted-foreground">
              {learner.emergencyContact.relationship} · {learner.emergencyContact.phone}
            </p>
          </div>
          <div>
            <p className="text-[11px] font-medium text-muted-foreground">Date of birth</p>
            <p className="text-[13px] text-foreground">
              {learner.dateOfBirth} ({learner.age} years)
            </p>
          </div>
          <div>
        ...
        <div className="space-y-3 p-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              ...
              className="h-8 text-[12px]"
              onClick={() =>
                toast.success("Printing card", {
                  description: "Sending the QR card to the default printer.",
                })
              }
            >
            ...
            <Button
              ...
              onClick={() =>
                toast.success("Download started", {
                  description: "The QR card image will download shortly.",
                })
              }
            >
            ...
            <Button
              size="sm"
              variant="outline"
              className="h-8 text-[12px] text-danger"
              onClick={onRevoke}
            >
            ...
            {loading ? (
              ...
              <ul className="space-y-2">
                {(credentials ?? []).map((c) => (
                  <li
                    key={c.id}
                    className="flex items-center justify-between gap-2 rounded-md bg-muted/40 px-2.5 py-1.5 text-[12px]"
                  >
...
function AttendanceTab({ learnerId }: { learnerId: string }) {
  const query = useQuery({
    ...
    queryFn: () => learnerService.attendance(learnerId),
  });
  ...
  if (!query.data || query.data.length === 0)
    return (
      <EmptyState
        ...
        description="No attendance events have been captured for this learner yet."
        icon={Calendar}
      />
    );
...
function AcademicsTab({ className }: { className: string }) {
  const subjectsQuery = useQuery({
    ...
    queryFn: () => academicsService.subjects(),
  });
  const assessmentsQuery = useQuery({
    queryKey: ["academics-assessments"],
    queryFn: () => academicsService.assessments(),
  });
  if (subjectsQuery.isLoading || assessmentsQuery.isLoading)
    return <TableSkeleton rows={5} columns={4} />;
  if (subjectsQuery.isError || assessmentsQuery.isError)
    return (
      <ErrorState
        onRetry={() => {
      ...
      <SectionCard title="Subjects" description={`Subjects offered to ${className}`}>
        {subjects.length === 0 ? (
          <EmptyState
            ...
            description="No subjects are mapped to this class yet."
            icon={Trophy}
          />
        ) : (
      ...
      <SectionCard title="Assessments" description="Class assessments and marks-entry status">
        {assessments.length === 0 ? (
          <EmptyState
            ...
            description="No assessments are scheduled for this class currently."
            icon={FileText}
          />
        ) : (
          <ul className="divide-y divide-border">
...
function ConductTab({ learnerId }: { learnerId: string }) {
  const query = useQuery({
    queryKey: ["learner-cases", learnerId],
    queryFn: () => supportService.casesForLearner(learnerId),
  });
  ...
  if (!query.data || query.data.length === 0)
    return (
      <EmptyState
        ...
        description="This learner has no recorded conduct cases."
        icon={ShieldCheck}
      />
    );
  return (
...
function WelfareTab({ learnerId }: { learnerId: string }) {
  const absencesQuery = useQuery({
    queryKey: ["learner-absences", learnerId],
    queryFn: () => supportService.absencesForLearner(learnerId),
  });
  const obsQuery = useQuery({
    queryKey: ["learner-observations", learnerId],
    queryFn: () => learnerService.observations(learnerId),
  });
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <SectionCard title="Authorised absences">
        {(absencesQuery.data ?? []).length === 0 ? (
          <EmptyState
            title="No authorised absences"
            description="No absences have been authorised for this learner."
            icon={Calendar}
          />
        ) : (
          <ul className="divide-y divide-border">
            {absencesQuery.data!.map((a) => (
              ...
              >
                <span className="text-foreground">
                  {a.category} · {a.fromDate} – {a.toDate}
                </span>
      ...
      <SectionCard title="Welfare concerns">
        {concerns.length === 0 ? (
          <EmptyState
            title="No welfare concerns recorded"
            description="No welfare concerns have been logged for this learner."
            icon={ShieldCheck}
          />
        ) : (
          <ul className="divide-y divide-border">
...
function HealthTab({ learnerId }: { learnerId: string }) {
  const query = useQuery({
    queryKey: ["learner-health", learnerId],
    queryFn: () => supportService.healthForLearner(learnerId),
  });
  if (query.isLoading) return <TableSkeleton rows={4} columns={4} />;
  if (query.isError) return <ErrorState onRetry={() => void query.refetch()} />;
  if (!query.data || query.data.length === 0)
    return (
      <EmptyState
        title="No health encounters"
        description="No sick bay visits have been recorded for this learner."
        icon={Stethoscope}
      />
    );
  return (
        ...
        {query.data.map((h) => (
          <li key={h.id} className="px-4 py-3 text-[12px]">
            ...
            <p className="mt-1 text-muted-foreground">
              {h.action} · Attended by {h.attendedBy}
            </p>
            <p className="mt-1 flex items-center gap-1 text-[11px] text-muted-foreground">
              <HeartPulse className="h-3 w-3" /> {h.arrivedAt}
            </p>
          </li>
        ))}
      </ul>
...
function ParticipationTab({ learnerId }: { learnerId: string }) {
  ...
  const items = (query.data ?? []).filter(
    (o) => o.category === "Positive conduct" || o.category === "General observation",
  );
  if (items.length === 0)
    return (
      <EmptyState
        title="No participation records"
        description="No clubs, sports or positive-conduct notes recorded yet."
        icon={Trophy}
      />
    );
  return (
...
function InterventionsTab({ learnerId }: { learnerId: string }) {
  const query = useQuery({
    ...
    queryFn: () => supportService.interventionsForLearner(learnerId),
  });
  if (query.isLoading) return <TableSkeleton rows={4} columns={4} />;
  if (query.isError) return <ErrorState onRetry={() => void query.refetch()} />;
  if (!query.data || query.data.length === 0)
    return (
      <EmptyState
        ...
        description="No support interventions are currently open for this learner."
        icon={ShieldCheck}
      />
    );
  return (
      ...
      <ul className="divide-y divide-border">
        {query.data.map((i) => (
          ...
          >
            ...
            <StatusBadge
              status={i.status}
              tone={i.status === "active" ? "info" : i.status === "overdue" ? "danger" : "success"}
            />
          </li>
        ))}
      </ul>
...
function DocumentsTab({ learnerId }: { learnerId: string }) {
  const query = useQuery({
    ...
    queryFn: () => learnerService.documents(learnerId),
  });
  if (query.isLoading) return <TableSkeleton rows={4} columns={3} />;
  if (query.isError) return <ErrorState onRetry={() => void query.refetch()} />;
  return (
    <SectionCard
      ...
      action={
        <Button
          ...
          className="h-8 text-[12px]"
          onClick={() =>
            toast("Upload document", {
              description: "Choose a file to attach to this learner's record.",
            })
          }
        >
    ...
    >
      {(query.data ?? []).length === 0 ? (
        <EmptyState
          title="No documents"
          description="No documents have been uploaded for this learner."
          icon={FileText}
        />
      ) : (
        <ul className="divide-y divide-border">
          {(query.data as LearnerDocument[]).map((d) => (
            <li
              key={d.id}
              className="flex items-center justify-between gap-2 px-4 py-2.5 text-[12px]"
            >
              <span className="min-w-0 truncate text-foreground">
                {d.name} <span className="text-muted-foreground">· {d.category}</span>
              </span>
              <span className="flex shrink-0 items-center gap-2 text-muted-foreground">
                ...
                <Button
                  ...
                  onClick={() => toast.success("Download started", { description: d.name })}
                >
...
function TimelineTab({ learnerId }: { learnerId: string }) {
  ...
  const events = [
    ...(obsQuery.data ?? []).map((o) => ({
      at: o.dateTime,
      label: `${o.category}: ${o.description}`,
      icon: FileText,
    })),
    ...(attQuery.data ?? []).slice(0, 10).map((a) => ({
      at: `${a.date} ${a.scanTime ?? ""}`.trim(),
      label: `${a.occasionName} — ${a.status}`,
      icon: IdCard,
    })),
  ]
  ...
  if (events.length === 0)
    return (
      <EmptyState
        ...
        description="No recorded activity for this learner yet."
        icon={Calendar}
      />
    );
  return (
